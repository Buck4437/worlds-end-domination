...
const buildings = function(player) {
    return {
        _data: [
            null, 
            {
        ...
        getBuilding(id) {
            ...
            return {
                ...
                multiplier() {
                    const apocalypseLevel = database.apocalypses(player).getApocalypseLevel();
                    const multiplier = new Decimal(1)
                        .times(database.upgrades(player).getUpgrade(1).apply())
                        .times(database.upgrades(player).getUpgrade(3).apply())
                        .times(database.upgrades(player).getUpgrade(5).apply())
                        .times(database.upgrades(player).getUpgrade(7).apply())
                        .times(database.upgrades(player).getUpgrade(10).apply())
                        .times(this.id === 1 ? database.upgrades(player).getUpgrade(2).apply() : 1)
                        .times(this.id === 2 ? database.upgrades(player).getUpgrade(4).apply() : 1)
                        .times(this.id === 3 ? database.upgrades(player).getUpgrade(6).apply() : 1)
                        .times(this.id === 4 ? database.upgrades(player).getUpgrade(8).apply() : 1)
                        .times(this.id === 4 ? database.upgrades(player).getUpgrade(9).apply() : 1)
